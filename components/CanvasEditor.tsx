      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-full max-w-2xl px-4 z-[110] flex flex-col items-center gap-2">
          ...
          {hasSelection && !isAiStreaming && (
             <div className="flex items-center gap-2 bg-white dark:bg-zinc-800 border border-border shadow-lg rounded-full px-4 py-1.5 animate-in fade-in slide-in-from-bottom-2 mb-1">
                 <Button variant="ghost" size="sm" onClick={handleCut} title={t('chat.cut')} className="h-8 w-8 px-0 rounded-full"><Scissors size={14}/></Button>
                 <div className="w-px h-4 bg-border"/>
                 <Button variant="ghost" size="sm" onClick={handleCopy} title={t('chat.copy')} className="h-8 w-8 px-0 rounded-full"><Copy size={14}/></Button>
                 <div className="w-px h-4 bg-border"/>
                 <Button variant="ghost" size="sm" onClick={handleDelete} title={t('chat.delete')} className="h-8 w-8 px-0 rounded-full text-destructive hover:text-destructive"><Trash2 size={14}/></Button>
             </div>
          )}
