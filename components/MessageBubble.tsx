            ...
            {!isUser && !isEditing && (
                <div className="flex flex-wrap items-center gap-4 mt-2 px-1">
                    <div className="flex items-center gap-3 text-[10px] text-muted-foreground uppercase tracking-wide font-medium select-none">
                        {metaDuration && (
                            <div className="flex items-center gap-1" title={t('chat.timeTaken')}>
                                <Clock size={10} />
                                <span>{formatDuration(metaDuration)}</span>
                            </div>
                        )}
                        {metaRounds !== undefined && (
                            <div className="flex items-center gap-1" title={t('chat.memoryRoundsN')}>
                                <Database size={10} />
                                <span>Mem: {metaRounds}</span>
                            </div>
                        )}
                        {metaMode && (
                            <div className="flex items-center gap-1" title={t('chat.modeUsed')}>
                                <Tag size={10} />
                    ...
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground">
                        <button onClick={() => onCopy(node.id, node.content)} title={t('chat.copy')} className="p-1 hover:text-foreground hover:bg-muted rounded transition-colors">
                            {copiedId === node.id ? <Check size={14} className="text-green-500"/> : <Copy size={14}/>}
                        </button>
                        
                        <button onClick={() => onToggleBookmark(node.id)} title={isBookmarked ? t('chat.removeBookmark') : t('chat.bookmarkChapter')} className={`p-1 hover:bg-muted rounded transition-colors ${isBookmarked ? 'text-yellow-500 hover:text-yellow-600' : 'hover:text-foreground'}`}>
                            <Star size={14} fill={isBookmarked ? "currentColor" : "none"}/>
                        </button>

                        <button onClick={() => onRegenerate(node)} title={t('chat.regenerate')} className="p-1 hover:text-foreground hover:bg-muted rounded transition-colors"><RotateCw size={14}/></button>

                        <button onClick={() => onEditStart(node.id, node.content)} title={t('chat.edit')} className="p-1 hover:text-foreground hover:bg-muted rounded transition-colors"><Edit2 size={14}/></button>

                        <button onClick={() => onCanvasStart(node.id)} title={t('chat.canvasMode')} className="p-1 hover:text-foreground hover:bg-muted rounded transition-colors"><Maximize2 size={14}/></button>

                        <button onClick={() => onBranch(node.id)} title={t('chat.branch')} className="p-1 hover:text-foreground hover:bg-muted rounded transition-colors"><GitBranch size={14}/></button>

                        <button onClick={() => onDelete(node.id)} title={t('chat.delete')} className="p-1 hover:text-destructive hover:bg-destructive/10 rounded transition-colors"><Trash2 size={14}/></button>

                        {metaMode === AppMode.Setting && (
                            <button
                                onClick={() => onAddToSetting(node.id, node.content)}
                                title={t('chat.addToSetting')}
