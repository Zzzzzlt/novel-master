...
import { Plus, Trash2, Settings, Download, Upload, MessageSquare, Moon, Sun, Laptop, X, Edit3, Check, FileText, HelpCircle, Loader2 } from 'lucide-react';
...
export const Sidebar: React.FC = () => {
  ...
  useEffect(() => {
    if (showSettings) {
       ...
       setDevModeInput(settings.developerMode || false);
       setLanguageInput(settings.language || 'zh');
       setShowReasoningInput(settings.showReasoning || false);
       // 同步模型配置
  ...
  const saveSettings = async () => {
    ...
    if (apiKeyInput && apiKeyInput !== settings.apiKey) {
      ...
      if (!result.valid) {
        setApiKeyValidationError(result.error || 'API Key 验证失败');
        return;
      }
    }

    // 同步 API Key 到 window 对象供 AI 服务读取
    ...
    `}>
      ...
      {showSettings && createPortal(
        <div className="fixed inset-0 z-[100] bg-background/80 backdrop-blur-sm flex items-center justify-center p-4">
           <div className="bg-card border border-border rounded-lg w-full max-w-md shadow-lg animate-in fade-in zoom-in duration-200 flex flex-col max-h-[90vh]">
             ...
             <div className="space-y-2">
               ...
               <input
                 type="password"
                 className={`w-full p-2 border rounded bg-background focus:outline-none focus:ring-1 ${
                   apiKeyValidationError ? 'border-destructive focus:ring-destructive' : 'border-input focus:ring-primary'
                 }`}
                 value={apiKeyInput}
                 onChange={e => { setApiKeyInput(e.target.value); setApiKeyValidationError(''); }}
               />
               {apiKeyValidationError && (
                 <p className="text-sm text-destructive">{apiKeyValidationError}</p>
               )}
             </div>
             ...
             <div className="p-6 border-t border-border flex gap-2 justify-end">
               <Button variant="outline" onClick={() => setShowSettings(false)} disabled={validatingApiKey}>{t('settings.cancel')}</Button>
               <Button onClick={saveSettings} disabled={validatingApiKey}>
                 {validatingApiKey ? (
                   <>
