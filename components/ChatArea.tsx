...
import {
  Send, ArrowDown, Menu, Info, Square, MessageSquarePlus, ArrowUpToLine, ArrowUp, X, Sparkles, Loader2, Star, Book, BookPlus
} from 'lucide-react';
...
// Helper to format API error messages for user-friendly display
function formatApiError(error: any): { title: string; description: string } {
  const message = error?.message || String(error);
  if (message.includes('401') || message.includes('Authentication') || message.includes('auth')) {
    return {
      title: t('dialog.authError'),
      description: t('dialog.authErrorDesc')
    };
  }
  if (message.includes('429') || message.includes('rate limit')) {
    return {
      title: t('dialog.rateLimitError'),
      description: t('dialog.rateLimitErrorDesc')
    };
  }
  return {
...
export const ChatArea: React.FC = () => {
  ...
  const [showSettingGuide, setShowSettingGuide] = useState(false); // 新手引导：设定模式提示
  const settingGuideTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  // 显示设定模式新手引导：第二轮对话AI回复完成后显示，15秒自动消失
  useEffect(() => {
    const session = sessions.find(s => s.id === currentSessionId);

    if (mode === AppMode.Setting && session && !loading) {
      // 统计AI回复数量（只有loading结束后才检查）
      const aiReplyCount = Object.values(session.messages).filter(
        (node: MessageNode) => node.role === Role.Assistant
      ).length;

      const hasSettingContent = session.context?.novelSetting?.trim();

      // 第二轮对话完成后显示（即有2条AI回复）
      if (aiReplyCount >= 2 && !hasSettingContent) {
        setShowSettingGuide(true);

        // 15秒后自动消失
        if (settingGuideTimerRef.current) clearTimeout(settingGuideTimerRef.current);
        settingGuideTimerRef.current = setTimeout(() => {
          setShowSettingGuide(false);
        }, 15000);
      }
    } else {
      setShowSettingGuide(false);
  ...
  }, [mode, currentSessionId, sessions, loading]);
  ...
  return (
    <div className="flex-1 flex flex-col h-full relative bg-background transition-all duration-300 w-full overflow-hidden">
      ...
      {/* 设定模式新手引导 - 位于导航栏下方 */}
      {showSettingGuide && mode === AppMode.Setting && (
        <div className="mx-auto max-w-2xl w-full px-4 pt-3 relative z-10" style={{ marginTop: chapterMarkers.length > 0 && showCompass ? '3rem' : '0' }}>
          <div className="relative bg-primary/10 border border-primary/20 rounded-lg p-4 animate-in fade-in slide-in-from-top-2 duration-300">
            <button
              onClick={() => setShowSettingGuide(false)}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h4 className="font-medium text-sm mb-1">{t('guide.settingModeTitle')}</h4>
                <p className="text-xs text-muted-foreground">{t('guide.settingModeDesc')}</p>
              </div>
            </div>
          </div>
        </div>
             ...
             {visibleHistory.length === 0 && (
                <div className="absolute inset-0 flex flex-col items-center justify-center text-muted-foreground/40 pointer-events-none select-none z-10">
                    <div className="text-center space-y-2 max-w-sm px-6 animate-in fade-in zoom-in duration-500">
                        ...
                        <p className="text-sm">
                            准备开始写作？使用设定模式生成设定集，随后使用故事模式开始创作吧！
                        </p>
                    </div>
                </div>
             )}
